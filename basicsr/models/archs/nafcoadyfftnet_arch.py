...
from .complexLayers import ComplexConv2d, ComplexBatchNorm2d, ComplexReLU, ComplexGeLU, ComplexSigmoid
...
"""ComplexODConv, Start"""
def complexodconv1x1(in_planes, out_planes, stride=1, reduction=0.0625, kernel_num=1):
    return ComplexODConv2d(in_planes, out_planes, kernel_size=1, stride=stride, padding=0,
...
class _Attention(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, groups=1, reduction=0.0625, kernel_num=4, min_channel=16):
        ...
        self.fc = ComplexConv2d(in_planes, attention_channel, 1, bias=False)
        self.bn = ComplexBatchNorm2d(attention_channel)
        self.relu = ComplexReLU(inplace=False)
        self.sigmoid = ComplexSigmoid()

        self.channel_fc = ComplexConv2d(attention_channel, in_planes, 1, bias=True)
        self.func_channel = self.get_channel_attention

        if in_planes == groups and in_planes == out_planes:  # depth-wise convolution
            self.func_filter = self.skip
        else:
            self.filter_fc = ComplexConv2d(attention_channel, out_planes, 1, bias=True)
            self.func_filter = self.get_filter_attention

        if kernel_size == 1:  # point-wise convolution
            self.func_spatial = self.skip
        else:
            self.spatial_fc = ComplexConv2d(attention_channel, kernel_size * kernel_size, 1, bias=True)
            self.func_spatial = self.get_spatial_attention

        if kernel_num == 1:
            self.func_kernel = self.skip
        else:
            self.kernel_fc = ComplexConv2d(attention_channel, kernel_num, 1, bias=True)
    ...
    def _initialize_weights(self):
        for m in self.modules():
            if isinstance(m, ComplexConv2d):
                ...
                if m.conv_r.bias is not None and m.conv_i.bias is not None:
    ...
    def get_channel_attention(self, x):
        channel_attention = self.sigmoid(self.channel_fc(x).view(x.size(0), -1, 1, 1) / self.temperature)
        return channel_attention

    def get_filter_attention(self, x):
        filter_attention = self.sigmoid(self.filter_fc(x).view(x.size(0), -1, 1, 1) / self.temperature)
        return filter_attention

    def get_spatial_attention(self, x):
        spatial_attention = self.spatial_fc(x).view(x.size(0), 1, 1, 1, self.kernel_size, self.kernel_size)
        spatial_attention = self.sigmoid(spatial_attention / self.temperature)
        return spatial_attention

    def get_kernel_attention(self, x):
        kernel_attention = self.kernel_fc(x).view(x.size(0), -1, 1, 1, 1, 1)
        kernel_attention = F.softmax(abs(kernel_attention) / self.temperature, dim=1)
...
class ComplexODConv2d(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=0, dilation=1, groups=1,
                 reduction=0.0625, kernel_num=4, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
        super(ComplexODConv2d, self).__init__()
        ...
        self.groups = groups
        self.kernel_num = kernel_num
        self.attention = _Attention(in_planes, out_planes, kernel_size, groups=groups,
                                   reduction=reduction, kernel_num=kernel_num)
        self.device = device
        ...
        self.weight_real = nn.Parameter(torch.randn(kernel_num, out_planes, in_planes//groups, kernel_size, kernel_size),
                                requires_grad=True).to(device)
        self.weight_imag = nn.Parameter(torch.randn(kernel_num, out_planes, in_planes//groups, kernel_size, kernel_size),
                                requires_grad=True).to(device)
        self.weight = torch.complex(self.weight_real, self.weight_imag)
        # self.bias = nn.Parameter(torch.zeros(kernel_num, out_planes), requires_grad=True)
        self.bias_real = nn.Parameter(torch.zeros(kernel_num, out_planes),
                                requires_grad=True).to(device)
        self.bias_imag = nn.Parameter(torch.zeros(kernel_num, out_planes),
                                requires_grad=True).to(device)
        self.bias = torch.complex(self.bias_real, self.bias_imag)
    ...
    def _forward_impl_common(self, x):
        ...
        #print(x.size(), spatial_attention, kernel_attention.shape, kernel_attention.is_cuda, self.weight.is_cuda)
        aggregate_weight = spatial_attention * kernel_attention * self.weight.unsqueeze(dim=0)
        aggregate_weight = torch.sum(aggregate_weight, dim=1).view(
            [-1, self.in_planes // self.groups, self.kernel_size, self.kernel_size])
        #print(self.bias.shape)
        aggregate_bias = spatial_attention * kernel_attention * self.bias.view([1, self.kernel_num, 1, 1, 1, -1])
        #print(aggregate_bias.shape)
        aggregate_bias = torch.sum(aggregate_bias, dim=1).view(
            [-1])
        # print(aggregate_weight.shape, aggregate_bias.shape)
        # print(aggregate_weight.real.shape, aggregate_bias.real.shape)
        # print("-------")
        
        # output = F.conv2d(x, weight=aggregate_weight, bias=aggregate_bias, stride=self.stride, padding=self.padding,
        #                   dilation=self.dilation, groups=self.groups * batch_size)
        output_real_1 = F.conv2d(x.real, weight=aggregate_weight.real.squeeze(dim=0), bias=aggregate_bias.real.squeeze(dim=0), stride=self.stride, padding=self.padding,
                          dilation=self.dilation, groups=self.groups * batch_size).type(torch.complex64)
        output_real_2 = F.conv2d(x.imag, weight=aggregate_weight.imag.squeeze(dim=0), bias=aggregate_bias.imag.squeeze(dim=0), stride=self.stride, padding=self.padding,
                          dilation=self.dilation, groups=self.groups * batch_size).type(torch.complex64)
        output_real = output_real_1 - output_real_2
        output_imag_1 = F.conv2d(x.real, weight=aggregate_weight.imag.squeeze(dim=0), bias=aggregate_bias.imag.squeeze(dim=0), stride=self.stride, padding=self.padding,
                          dilation=self.dilation, groups=self.groups * batch_size).type(torch.complex64)
        output_imag_2 = F.conv2d(x.imag, weight=aggregate_weight.real.squeeze(dim=0), bias=aggregate_bias.real.squeeze(dim=0), stride=self.stride, padding=self.padding,
                          dilation=self.dilation, groups=self.groups * batch_size).type(torch.complex64)
        output_imag = output_imag_1 + output_imag_2
        output = output_real.type(torch.complex64) + 1j * output_imag.type(torch.complex64)
    ...
    def _forward_impl_pw1x(self, x):
        channel_attention, filter_attention, spatial_attention, kernel_attention = self.attention(x)
        x = x * channel_attention
        # output = F.conv2d(x, weight=self.weight.squeeze(dim=0), bias=self.bias.squeeze(dim=0), stride=self.stride, padding=self.padding,
        #                   dilation=self.dilation, groups=self.groups)
        output_real = F.conv2d(x.real, weight=self.weight_real.squeeze(dim=0), bias=self.bias_real.squeeze(dim=0), stride=self.stride, padding=self.padding,
                          dilation=self.dilation, groups=self.groups) - \
                      F.conv2d(x.imag, weight=self.weight_imag.squeeze(dim=0), bias=self.bias_imag.squeeze(dim=0), stride=self.stride, padding=self.padding,
                          dilation=self.dilation, groups=self.groups)
        output_imag = F.conv2d(x.real, weight=self.weight_imag.squeeze(dim=0), bias=self.bias_imag.squeeze(dim=0), stride=self.stride, padding=self.padding,
                          dilation=self.dilation, groups=self.groups) + \
                      F.conv2d(x.imag, weight=self.weight_real.squeeze(dim=0), bias=self.bias_real.squeeze(dim=0), stride=self.stride, padding=self.padding,
                          dilation=self.dilation, groups=self.groups)
        output = output_real.type(torch.complex64) + 1j * output_imag.type(torch.complex64)
        output = output * filter_attention
        return output
...
class fft_bench_complex_mlp(nn.Module):
    def __init__(self, dim, dw=1, norm='backward', act_method=nn.ReLU, window_size=None, bias=False):
        ...
        hid_dim = dim * dw
        # print(dim, hid_dim)
        self.odconv1 = complexodconv1x1(dim, hid_dim, kernel_num=1)
        self.odconv2 = complexodconv1x1(hid_dim, dim, kernel_num=1)
...
class NAFCoAdyFFTBlock(nn.Module):
    def __init__(self, c, num_heads=1, window_size=8, window_size_fft=-1, shift_size=-1, DW_Expand=2, FFN_Expand=2,
        ...
        # Simplified Channel Attention, for NAFNet
        ...
        # Coordinate Attention, for NAFCoANet
        ...
        if window_size_fft is None or window_size_fft >= 0:
            self.fft_block1 = fft_bench_complex_mlp(c, DW_Expand, window_size=window_size_fft, bias=True, act_method=ComplexGeLU) # , act_method=nn.GELU
            self.fft_block2 = fft_bench_complex_mlp(c, DW_Expand, window_size=window_size_fft, bias=True, act_method=ComplexGeLU)
    ...
    def forward(self, inp):
        ...
        if self.window_size_fft is None or self.window_size_fft >= 0:
            ...
            if self.sin:
                ...
                x = torch.sin(self.fft_block2(y)) + self.conv5(self.sg(self.conv4(x))) # * self.gamma
            else:
...
class NAFCoAdyFFTNet(nn.Module):
    ...
    def __init__(self, img_channel=3, width=32, middle_blk_num=1, enc_blk_nums=[1, 1, 1, 28], dec_blk_nums=[1, 1, 1, 1],
        ...
        for i in range(len(enc_blk_nums)):
            self.encoders.append(
                nn.Sequential(
                    *[NAFCoAdyFFTBlock(chan, num_heads_e[i], window_size_e[i], window_size_e_fft[i], window_sizex_e[i]) for _ in range(enc_blk_nums[i])]
                )
            )
            self.downs.append(
        ...
        self.middle_blks = \
            nn.Sequential(
                *[NAFCoAdyFFTBlock(chan, num_heads_m[0], window_size_m[0], window_size_m_fft[0], window_sizex_m[0]) for _ in range(middle_blk_num)]
            )
        ...
        for j in range(len(dec_blk_nums)):
            ...
            self.decoders.append(
                nn.Sequential(
                    *[NAFCoAdyFFTBlock(chan, num_heads_d[j], window_size_d[j], window_size_d_fft[j], window_sizex_d[j]) for _ in range(dec_blk_nums[j])]
                )
            )
