...
class AvgPool2d(nn.Module):
    def __init__(self, kernel_size=None, base_size=None, auto_pad=True, fast_imp=False, train_size=None, axis=None):
        super().__init__()
    ...
    def forward(self, x):
        N, _, _, _, = x.shape
        if self.axis == 'height':
            x = rearrange(x, 'N C H W -> (N H) C 1 W')
        elif self.axis == 'column':
            x = rearrange(x, 'N C H W -> (N W) C H 1')
        
        #print(self.kernel_size, self.base_size)
        #print('--')
        if self.kernel_size is None and self.base_size:
            ...
            #print(self.base_size, self.train_size, self.kernel_size)
            # only used for fast implementation
            self.max_r1 = max(1, self.rs[0] * x.shape[2] // train_size[-2])
            self.max_r2 = max(1, self.rs[0] * x.shape[3] // train_size[-1])
        # else:
        #     _,_,H,W = x.shape
        #     self.kernel_size[0] = H if self.kernel_size[0] == None else self.kernel_size[0]
        #     self.kernel_size[1] = W if self.kernel_size[1] == None else self.kernel_size[1]
                
        if self.kernel_size[0] >= x.size(-2) and self.kernel_size[1] >= x.size(-1):
            x = F.adaptive_avg_pool2d(x, 1)
            if self.axis == 'height':
                x = rearrange(x, '(N H) C 1 W -> N C H W', N=N)
            elif self.axis == 'column':
                x = rearrange(x, '(N W) C H 1 -> N C H W', N=N)
            return x
        ...
        if self.axis == 'height':
            out = rearrange(out, '(N H) C 1 W -> N C H W', N=N)
        elif self.axis == 'column':
            out = rearrange(out, '(N W) C H 1 -> N C H W', N=N)
        return out
...
def replace_layers(model, base_size, train_size, fast_imp, **kwargs):
    for n, m in model.named_children():
        ...
        # if isinstance(m, nn.AdaptiveAvgPool2d) and m.output_size == (1, None): # Only Pool through dim=1
        #     pool = AvgPool2d(base_size=base_size, fast_imp=fast_imp, train_size=train_size, axis='column')
        #     assert m.output_size == (1, None), m.output_size
        #     setattr(model, n, pool)

        # if isinstance(m, nn.AdaptiveAvgPool2d) and m.output_size == (None, 1): # Only Pool through dim=0
        #     pool = AvgPool2d(base_size=base_size, fast_imp=fast_imp, train_size=train_size, axis='height')
        #     assert m.output_size == (None, 1), m.output_size
        #     setattr(model, n, pool)

        if isinstance(m, nn.AdaptiveAvgPool2d) and m.output_size == 1: # Custom Global AvgPool2d
            pool = AvgPool2d(base_size=base_size, fast_imp=fast_imp, train_size=train_size)
            assert m.output_size == 1, m.output_size
            setattr(model, n, pool)
...
class Local_Base():
    def convert(self, *args, train_size, **kwargs):
        ...
        flow = torch.rand(1, 2, train_size[-2], train_size[-1])
        with torch.no_grad():
            self.forward(imgs, flow)
...
class SKFF(nn.Module):
    ...
    def forward(self, inp_feats):
        ...
        # [tensor(N,C,H,W) tensor(N,C,H,W)] -> (N, 2*C, H, W)
        ...
        # (N, C, H, W) -> (N, C, 1, 1) if 'global pool' else (N, C, H, W)
        feats_S = self.avg_pool(feats_U)
        H, W = feats_S.shape[-2], feats_S.shape[-1]

        # (N, C, 1, 1) -> (N, d, 1, 1)
        # (N, C, H, W) -> (N, d, H, W)
        feats_Z = self.conv_du(feats_S)

        # (N, d, 1, 1) -> [(N, C, 1, 1), (N, C, 1, 1)]
        # (N, d, H, W) -> [(N, C, H, W), (N, C, H, W)]
        attention_vectors = [fc(feats_Z) for fc in self.fcs]

        # [(N, C, 1, 1), (N, C, 1, 1)] -> (N, 2*C, 1, 1)
        ...
        # (N, 2*C, 1, 1) -> (N, 2, C, 1, 1)
        # (N, 2*C, H, W) -> (N, 2, C, H, W)
        attention_vectors = attention_vectors.view(batch_size, self.height, n_feats, H, W)
        
        # softmax along dim=1, i.e, softmax between 2 elements everytime
...
class NAFCoAFFTBlock(nn.Module):
...
class NAFCoAFFTFlowNet(nn.Module):
    ...
    def __init__(self, img_channel=3, width=32, middle_blk_num=1, enc_blk_nums=[1, 1, 1, 28], dec_blk_nums=[1, 1, 1, 1],
        ...
        # self.skff.append(SKFF(width, 2))
        # self.skff.append(SKFF(width * 2, 2))
        # self.skff.append(SKFF(width * 4, 2))

        # self.skffup = nn.ModuleList()
        # self.skffup.append(
        #     nn.Sequential(
        #         nn.Conv2d(width * 2, width, 1, stride=1, padding=0),
        #         nn.Upsample(scale_factor=2, mode='bilinear')
        #     ))
        # self.skffup.append(
        #     nn.Sequential(
        #         nn.Conv2d(width * 4, width * 2, 1, stride=1, padding=0),
        #         nn.Upsample(scale_factor=2, mode='bilinear')
        #     ))
        # self.skffup.append(
        #     nn.Sequential(
        #         nn.Conv2d(width * 8, width * 4, 1, stride=1, padding=0),
        #         nn.Upsample(scale_factor=2, mode='bilinear')
        #     ))
        # self.AFFs = nn.ModuleList([
        #     AFF(width * 15, width*1),
        #     AFF(width * 15, width*2),
        #     AFF(width * 15, width*4),
        # ])
        ...
        for i in range(len(enc_blk_nums)):
            self.encoders.append(
                nn.Sequential(
                    *[NAFCoAFFTBlock(chan, num_heads_e[i], window_size_e[i], window_size_e_fft[i], window_sizex_e[i]) for _ in range(enc_blk_nums[i])]
                )
            )
            self.downs.append(
        ...
        self.middle_blks = \
            nn.Sequential(
                *[NAFCoAFFTBlock(chan, num_heads_m[0], window_size_m[0], window_size_m_fft[0], window_sizex_m[0]) for _ in range(middle_blk_num)]
            )
        ...
        for j in range(len(dec_blk_nums)):
            ...
            #self.skff.append(SKFF(chan, 2))

            self.decoders.append(
                nn.Sequential(
                    *[NAFCoAFFTBlock(chan, num_heads_d[j], window_size_d[j], window_size_d_fft[j], window_sizex_d[j]) for _ in range(dec_blk_nums[j])]
                )
            )
    ...
    def forward(self, inp, flow):
        ...
        # skffouts = []
        # skffouts.append(encs[3])
        # skffouts.append(self.skff[2]([encs[2], self.skffup[2](skffouts[-1])]))
        # skffouts.append(self.skff[1]([encs[1], self.skffup[1](skffouts[-1])]))
        # skffouts.append(self.skff[0]([encs[0], self.skffup[0](skffouts[-1])]))
...
class NAFCoAFFTFlowNetLocal(Local_Base, NAFCoAFFTFlowNet):
    def __init__(self, *args, train_size=(1, 1, 128, 128), base_size=192, fast_imp=False, **kwargs):
        Local_Base.__init__(self)
        NAFCoAFFTFlowNet.__init__(self, *args, **kwargs)
...
if __name__ == '__main__':
    ...
    net = NAFCoAFFTFlowNet(img_channel=img_channel, width=width, middle_blk_num=middle_blk_num,
    ...
    # from time_utils.timer import Timer

    # inp = torch.rand(1, 1, 1080, 1920).cuda()
    # flow = torch.rand(1, 2, 1080, 1920).cuda()
    # net.cuda()
    # net.eval()
