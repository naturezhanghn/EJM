...
class Sparsity_Estimator(nn.Module):
    def __init__(self, in_channel=1, dim=28, expand=2, sparse=False):
        ...
        # Input projection
        self.in_proj = nn.Conv2d(in_channel, dim, 1, 1, 0, bias=False) # nn.Conv2d(28, dim, 1, 1, 0, bias=False)
...
class CST(nn.Module):
    def __init__(self, dim=28, stage=2, num_blocks=[2, 2, 2], sparse=False):
        ...
        # Sparsity Estimator
        if num_blocks==[2,4,6]:
            self.fe = nn.Sequential(Sparsity_Estimator(in_channel=1, dim=28,expand=2,sparse=False),
                        Sparsity_Estimator(in_channel=28, dim=28, expand=2, sparse=sparse))
        else:
        ...
        # Output projection
        self.out_proj = nn.Conv2d(self.dim, 1, 3, 1, 1, bias=False) # nn.Conv2d(self.dim, dim, 3, 1, 1, bias=False)
...
if __name__ == '__main__':
    ...
    net = CST(num_blocks=[2, 4, 6], sparse=False).cuda()
    inp_shape = (1, 256, 256)
