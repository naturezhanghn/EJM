...
class MST(nn.Module):
    def __init__(self, dim=28, stage=3, num_blocks=[2,2,2]):
        ...
        # Input projection
        self.embedding = nn.Conv2d(1, self.dim, 3, 1, 1, bias=False) # nn.Conv2d(28, self.dim, 3, 1, 1, bias=False)
        ...
        # Output projection
        self.mapping = nn.Conv2d(self.dim, 1, 3, 1, 1, bias=False) # nn.Conv2d(self.dim, 28, 3, 1, 1, bias=False)
    ...
    def forward(self, x, mask=None):
        ...
        """
        if mask == None:
            mask = torch.zeros((1,28,128,182)).cuda() # torch.zeros((1,28,256,310)).cuda()
...
if __name__ == '__main__':
    ...
    net = MST(dim=28, stage=2, num_blocks=[4, 7, 5]).cuda()
    inp_shape = (1, 256, 256)
