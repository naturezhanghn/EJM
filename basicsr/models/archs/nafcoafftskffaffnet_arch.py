...
class NAFCoAFFTSKFFAFFNet(nn.Module):
    ...
    def forward(self, inp):
        ...
        z14 = F.interpolate(z12, scale_factor=0.5)
        z21 = F.interpolate(encs[1], scale_factor=2)
        z24 = F.interpolate(encs[1], scale_factor=0.5)
        z42 = F.interpolate(encs[2], scale_factor=2)
        z41 = F.interpolate(z42, scale_factor=2)
        z84 = F.interpolate(encs[3], scale_factor=2)
        z82 = F.interpolate(z84, scale_factor=2)
        z81 = F.interpolate(z82, scale_factor=2)

        AFFouts.append(encs[3])
        AFFouts.append(self.drop3(self.AFFs[2](z14, z24, encs[2], z84)))
        AFFouts.append(self.drop2(self.AFFs[1](z12, encs[1], z42, z82)))
        AFFouts.append(self.drop1(self.AFFs[0](encs[0], z21, z41, z81)))

        x = self.middle_blks(x)

        for decoder, up, enc_skip, skff in zip(self.decoders, self.ups, AFFouts, self.skff):
            x = up(x)
            # x = x + enc_skip
            x = skff([enc_skip, x])
            x = decoder(x)
...
class NAFCoAFFTSKFFAFFNetLocal(Local_Base, NAFCoAFFTSKFFAFFNet):
    def __init__(self, *args, train_size=(1, 1, 128, 128), base_size=192, fast_imp=False, **kwargs):
        Local_Base.__init__(self)
        NAFCoAFFTSKFFAFFNet.__init__(self, *args, **kwargs)
