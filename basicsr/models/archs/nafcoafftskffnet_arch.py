...
class NAFCoAFFTSKFFNet(nn.Module):
    ...
    def forward(self, inp):
        ...
        for decoder, up, enc_skip, skff in zip(self.decoders, self.ups, encs[::-1], self.skff):
            x = up(x)
            # x = x + enc_skip
            x = skff([enc_skip, x])
            x = decoder(x)
...
class NAFCoAFFTSKFFNetLocal(Local_Base, NAFCoAFFTSKFFNet):
    def __init__(self, *args, train_size=(1, 1, 128, 128), base_size=192, fast_imp=False, **kwargs):
        Local_Base.__init__(self)
        NAFCoAFFTSKFFNet.__init__(self, *args, **kwargs)
