...
from torch.nn.functional import relu, gelu, max_pool2d, avg_pool2d, dropout, dropout2d, interpolate, tanh
import torch
...
def complex_relu(input, inplace=False):
    outp_real = relu(input.real, inplace=inplace)
    outp_imag = relu(input.imag, inplace=inplace)
    return outp_real.type(torch.complex64)+1j*outp_imag.type(torch.complex64)

def complex_gelu(input):
    outp_real = gelu(input.real)
    outp_imag = gelu(input.imag)
    return outp_real.type(torch.complex64)+1j*outp_imag.type(torch.complex64)

def complex_sigmoid(input):
    outp_real = torch.sigmoid(input.real)
    outp_imag = torch.sigmoid(input.imag)
    return outp_real.type(torch.complex64)+1j*outp_imag.type(torch.complex64)
