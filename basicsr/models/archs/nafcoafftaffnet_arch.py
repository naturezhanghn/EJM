...
class NAFCoAFFTAFFNet(nn.Module):
    ...
    def forward(self, inp):
        ...
        for decoder, up, enc_skip in zip(self.decoders, self.ups, AFFouts):
            x = up(x)
            x = x + enc_skip
            # x = skff([enc_skip, x])
            x = decoder(x)
...
class NAFCoAFFTAFFNetLocal(Local_Base, NAFCoAFFTAFFNet):
    def __init__(self, *args, train_size=(1, 1, 128, 128), base_size=192, fast_imp=False, **kwargs):
        Local_Base.__init__(self)
        NAFCoAFFTAFFNet.__init__(self, *args, **kwargs)
