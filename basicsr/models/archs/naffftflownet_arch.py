    ...
    def __init__(self, img_channel=3, width=32, middle_blk_num=1, enc_blk_nums=[1, 1, 1, 28], dec_blk_nums=[1, 1, 1, 1],
        ...
        for i in range(len(enc_blk_nums)):
            self.encoders.append(
                nn.Sequential(
                    *[NAFFFTBlock(chan, num_heads_e[i], window_size_e[i], window_size_e_fft[i], window_sizex_e[i]) for _ in range(enc_blk_nums[i])]
                )
            )
            self.downs.append(
        ...
        self.middle_blks = \
            nn.Sequential(
                *[NAFFFTBlock(chan, num_heads_m[0], window_size_m[0], window_size_m_fft[0], window_sizex_m[0]) for _ in range(middle_blk_num)]
            )
        ...
        for j in range(len(dec_blk_nums)):
            ...
            self.decoders.append(
                nn.Sequential(
                    *[NAFFFTBlock(chan, num_heads_d[j], window_size_d[j], window_size_d_fft[j], window_sizex_d[j]) for _ in range(dec_blk_nums[j])]
                )
            )
...
def prepare_input(resolution):
    ...
    x2 = torch.FloatTensor(1, 2, 128, 128)
...
if __name__ == '__main__':
    ...
    net = NAFFFTFlowNet(img_channel=img_channel, width=width, middle_blk_num=middle_blk_num,
