...
class NAFCoAFFTSKFF2FlowNet(nn.Module):
    ...
    def forward(self, inp, flow):
        ...
        for decoder, up, enc_skip in zip(self.decoders, self.ups, skffouts):
            x = up(x)
            x = x + enc_skip
            # x = skff([enc_skip, x])
            x = decoder(x)
...
class NAFCoAFFTSKFF2FlowNetLocal(Local_Base, NAFCoAFFTSKFF2FlowNet):
    def __init__(self, *args, train_size=(1, 1, 128, 128), base_size=192, fast_imp=False, **kwargs):
        Local_Base.__init__(self)
        NAFCoAFFTSKFF2FlowNet.__init__(self, *args, **kwargs)
