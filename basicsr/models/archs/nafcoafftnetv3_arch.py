...
class fft_bench_complex_mlp(nn.Module):
    def __init__(self, dim, dw=1, norm='backward', act_method=nn.ReLU, window_size=None, bias=False):
        ...
        hid_dim = dim * dw
        # print(dim, hid_dim)
        self.complex_weight1_real = nn.Parameter(torch.Tensor(dim, 2*hid_dim))
        self.complex_weight1_imag = nn.Parameter(torch.Tensor(dim, 2*hid_dim))
        ...
        if bias:
            self.b1_real = nn.Parameter(torch.zeros((1, 1, 1, 2*hid_dim)), requires_grad=True)
            self.b1_imag = nn.Parameter(torch.zeros((1, 1, 1, 2*hid_dim)), requires_grad=True)
    ...
    def forward(self, x):
        ...
        y = torch.cat([y.real, y.imag], dim=dim) # (real1 real2 imag1 imag2)
        # print(y.shape)
        chunks = torch.chunk(y, 4, dim=-1)
        # print(y.shape)
        y = torch.cat((chunks[0], chunks[2], chunks[1], chunks[3]), dim=-1) # (real1 real2 imag1 imag2) -> (real1 imag1 real2 imag2)
        # print(y.shape)
...
class NAFCoAFFTBlock(nn.Module):
    def __init__(self, c, num_heads=1, window_size=8, window_size_fft=-1, shift_size=-1, DW_Expand=2, FFN_Expand=2,
        ...
        if window_size_fft is None or window_size_fft >= 0:
            self.fft_block1 = fft_bench_complex_mlp(c, DW_Expand, window_size=window_size_fft, bias=True, act_method=SimpleGate) # , act_method=nn.GELU
            self.fft_block2 = fft_bench_complex_mlp(c, DW_Expand, window_size=window_size_fft, bias=True, act_method=SimpleGate)
...
class NAFCoAFFTNetv3(nn.Module):
