...
class DFFN(nn.Module):
    def __init__(self, dim, ffn_expansion_factor, bias):
        ...
        self.patch_size = 8
        self.padder_size = self.patch_size
...
class NAFCoAFFT2Block(nn.Module):
    def __init__(self, c, num_heads=1, window_size=8, window_size_fft=-1, shift_size=-1, DW_Expand=2, FFN_Expand=2,
        ...
        self.dffn = dffn
        if self.dffn:
            self.dffn = DFFN(dim=c, ffn_expansion_factor=3, bias=False)
    ...
    def __init__(self, img_channel=3, width=32, middle_blk_num=1, enc_blk_nums=[1, 1, 1, 28], dec_blk_nums=[1, 1, 1, 1],
        ...
        for i in range(len(enc_blk_nums)):
            self.encoders.append(
                nn.Sequential(
                    *[NAFCoAFFT2Block(chan, num_heads_e[i], window_size_e[i], window_size_e_fft[i], window_sizex_e[i]) for _ in range(enc_blk_nums[i])]
                )
            )
            self.downs.append(
        ...
        self.middle_blks = \
            nn.Sequential(
                *[NAFCoAFFT2Block(chan, num_heads_m[0], window_size_m[0], window_size_m_fft[0], window_sizex_m[0], dffn=True) for _ in range(middle_blk_num)]
            )
        ...
        for j in range(len(dec_blk_nums)):
            ...
            self.decoders.append(
                nn.Sequential(
                    *[NAFCoAFFT2Block(chan, num_heads_d[j], window_size_d[j], window_size_d_fft[j], window_sizex_d[j]) for _ in range(dec_blk_nums[j])]
                )
            )
