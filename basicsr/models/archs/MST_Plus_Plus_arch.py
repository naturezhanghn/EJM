...
class MST_Plus_Plus(nn.Module):
    def __init__(self, in_channels=3, out_channels=28, n_feat=28, stage=3):
        ...
        # self.fution = nn.Conv2d(2, 1, 1, padding=0, bias=True) # nn.Conv2d(56, 28, 1, padding=0, bias=True)
    ...
    def forward(self, x):
        ...
        """
        # if Phi==None:
        #     Phi = torch.rand((1,1,128,128)).cuda() # Phi = torch.rand((1,28,256,256)).cuda()
        ...
        # x = self.conv_in(x)
        x_latent = self.conv_in(x)
        h = self.body(x_latent)
        h = self.conv_out(h)
        h += x
...
if __name__ == '__main__':
    ...
    net = MST_Plus_Plus(in_channels=1, out_channels=1, n_feat=28, stage=3).cuda()
    inp_shape = (1, 256, 256)
