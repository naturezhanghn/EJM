...
class ImageMlossFlowModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        for preflow in preflows:
            ...
            # print(self.opt['train'].get('flow_weight', 1.0))
...
class ImageMultiMlossModel(ImageMlossModel):
    # For MIMOUNet
    def optimize_parameters(self, current_iter):
        self.optimizer_g.zero_grad()
        preds = self.net_g(self.lq)
        ...
        for i, pred in enumerate(preds):
            self.cur_gt = F.interpolate(self.gt, scale_factor=2 ** i / 4., mode='bilinear')
            l_pix += self.cri_pix(pred, self.cur_gt)
            if self.opt['train'].get('flow_opt'):
                l_pix += self.cri_flow(pred, self.cur_gt)
            if self.opt['train'].get('msfr_opt'):
                l_pix += self.cri_msfr(pred, self.cur_gt)
...
class StripformerModel(ImageMlossModel):
    # For Sharpformer
    def optimize_parameters(self, current_iter):
        self.optimizer_g.zero_grad()
        preds = self.net_g(self.lq)
