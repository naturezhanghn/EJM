...
class REMOTEDATASet(data.Dataset):
    def __init__(self, opt):
        ...
        self.opt = opt
        print(opt) 
        self.path = sorted(glob.glob(os.path.join(Path(opt['dataroot_gt'])) + "/*.*")) # + sorted(glob.glob(os.path.join(Path(opt['dataroot_gt'])) + "/*.*"))[:2000]
...
class REMOTEDATASetNoWarp(data.Dataset):
    def __init__(self, opt):
        super(REMOTEDATASetNoWarp, self).__init__()
        self.opt = opt
        print(opt) 
        self.path = sorted(glob.glob(os.path.join(Path(opt['dataroot_gt'])) + "/*.*")) # + sorted(glob.glob(os.path.join(Path(opt['dataroot_gt'])) + "/*.*"))[:2000]
        self.transform = transforms.Compose( [transforms.ToTensor(),])
