...
def warp(img,jit):
    ...
    img = torch.from_numpy(img).float() # val psnr error, np.int32 pr np.float32 instead of np.int16
    h, w = img.shape
...
parser.add_argument('--weights', default='/raid/chenzd/project/restore_sateline/experiments/Deblurring_JRNet_20230809_480x640Dv1plus20uv_LAP_retrain3_archived_20230830_123159/models/net_g_latest.pth', type=str, help='Path to weights')
parser.add_argument('--dataset', default='RealData_JARNet_lijiang', type=str, help='Test Dataset') # ['GoPro', 'HIDE', 'RealBlur_J', 'RealBlur_R']
...
files = glob(os.path.join(inp_dir, '*.png'))
print(len(files))
with torch.no_grad():
    for file_ in tqdm(files):
        ...
        flow = np.load('lijiang_pitch_plus0.npy')
        img_tdi = gamma_correction(img_tdi.copy() , 1/2.2 ,inv=True)
        print(flow.shape)
        # img_tdi = warp(img_tdi,flow)
        print(img_tdi.shape)
        # utils.save_img((os.path.join(result_dir, os.path.splitext(os.path.split(file_)[-1])[0]+'_sr_coafft_warp.png')), img_as_ubyte(img_tdi))
        ...
        utils.save_img((os.path.join(result_dir, os.path.splitext(os.path.split(file_)[-1])[0]+'_sr_coafft.png')), img_as_ubyte(restored))
